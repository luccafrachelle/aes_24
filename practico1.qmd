...
```{r}
df_split = initial_split(df, prop = 0.8, strata = Private)
...
```{r}
least_squares = lm_fit %>% predict(df_test) %>% bind_cols(df_test) %>% 
...
```
```{r}
ridge_grid %>% select_best(metric = "rmse")
```
...
```{r}
ridge = ridge_final %>% fit(df_train) %>% predict(df_test) %>% bind_cols(df_test) %>% 
  metrics(truth = Apps, estimate = .pred)
```
...
```{r}
lasso = lasso_final %>% fit(df_train) %>% predict(df_test) %>% bind_cols(df_test) %>% 
  metrics(truth = Apps, estimate = .pred)
```
...
```{r}
knn_grid %>% select_best(metric = "rmse")
```
...
```{r}
 knn = knn_final %>% fit(df_train) %>% predict(df_test) %>% bind_cols(df_test) %>% 
  metrics(truth = Apps, estimate = .pred)
```
